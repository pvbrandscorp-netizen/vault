...
export default function Vault() {
  ...
  const [df, setDf] = useState<AnyState>({ name: "", type: "credit_card", scope: "personal", creditLimit: "", outstanding: "", dueAmount: "", dueDate: "", currency: "PHP", notes: "" });
  ...
  const [axf, setAxf] = useState<AnyState>({ name: "", category: "vehicle", purchasePrice: "", purchaseDate: "", usefulLife: "5", salvageValue: "0", marketValue: "", scope: "personal", currency: "PHP", notes: "" });
  ...
  const save = useCallback((a?: AnyState, d?: AnyState, r?: AnyState, c?: AnyState, t?: AnyState, sn?: AnyState, p?: AnyState, ax?: AnyState, iv?: AnyState, pnl?: AnyState) => {
    const data = {
      accounts: a ?? accounts, debts: d ?? debts, receivables: r ?? receivables,
      cryptos: c ?? cryptos, txns: t ?? txns, pipelines: p ?? pipelines,
      assets: ax ?? assets, inventory: iv ?? inventory, pnlBatches: pnl ?? pnlBatches,
      snaps: sn ?? snaps, rates
    };
    if (!userId) return;
  ...
  }, [accounts, debts, receivables, cryptos, txns, pipelines, assets, inventory, pnlBatches, snaps, rates, userId, saveToSupabase]);
  ...
  const snap = useCallback((a?: AnyState, d?: AnyState, c?: AnyState) => {
    const aa = a || accounts, dd = d || debts, cc = c || cryptos;
    const ast = aa.reduce((s: number, x: AnyState) => s + x.balance * (rates[x.currency] || 1), 0);
    const dbt = dd.reduce((s: number, x: AnyState) => s + x.outstanding * (rates[x.currency] || 1), 0);
    const cry = cc.reduce((s: number, x: AnyState) => s + x.amount * (rates[x.coin] || 0), 0);
    const now = new Date();
    ...
    const ns = [...snaps, { ts, label, a: ast, db: dbt, c: cry, n: ast + cry - dbt }].slice(-200);
    setSnaps(ns); return ns;
  }, [accounts, debts, cryptos, rates, snaps]);

  // Scoped
  const sA = scope === "all" ? accounts : accounts.filter((a: AnyState) => a.scope === scope);
  const sD = scope === "all" ? debts : debts.filter((d: AnyState) => d.scope === scope);
  const sR = scope === "all" ? receivables : receivables.filter((r: AnyState) => r.scope === scope);
  const sC = scope === "all" ? cryptos : cryptos.filter((c: AnyState) => c.scope === scope);
  const sT = scope === "all" ? txns : txns.filter((t: AnyState) => { const a = accounts.find((x: AnyState) => x.id === t.accountId); return a && a.scope === scope; });
  const sP = scope === "all" ? pipelines : pipelines.filter((p: AnyState) => p.scope === scope);
  const activeP = sP.filter((p: AnyState) => p.status === "active");
  const sAx = scope === "all" ? assets : assets.filter((a: AnyState) => a.scope === scope);
  const sIv = scope === "all" ? inventory : inventory.filter((i: AnyState) => i.scope === scope);
  const sPnl = scope === "all" ? pnlBatches : pnlBatches.filter((b: AnyState) => b.scope === scope);

  const getBookValue = (a: AnyState) => {
    if (!a.purchaseDate || !a.usefulLife) return a.purchasePrice || 0;
  ...
  // ── CRUD ──
  const addAccount = () => { if (!af.name || !rawN(af.balance)) return; const na = [...accounts, { id: gid(), ...af, balance: parseN(af.balance) }]; setAccounts(na); const ns = snap(na); save(na, null, null, null, null, ns); setAf({ name: "", category: "bank", scope: "personal", balance: "", currency: "PHP", notes: "" }); setSf(null); };
  const saveEditAcc = () => { const na = accounts.map((a: AnyState) => a.id === eId ? { ...a, ...af, balance: parseN(af.balance) } : a); setAccounts(na); const ns = snap(na); save(na, null, null, null, null, ns); setEId(null); setSf(null); };
  const delAcc = (id: string) => { const na = accounts.filter((a: AnyState) => a.id !== id); const nt = txns.filter((t: AnyState) => t.accountId !== id && t.toAccountId !== id); setAccounts(na); setTxns(nt); save(na, null, null, null, nt); };

  const addDebt = () => { if (!df.name) return; const nd = [...debts, { id: gid(), ...df, creditLimit: parseN(df.creditLimit), outstanding: parseN(df.outstanding), dueAmount: parseN(df.dueAmount), lastUp: new Date().toISOString() }]; setDebts(nd); const ns = snap(null, nd); save(null, nd, null, null, null, ns); setDf({ name: "", type: "credit_card", scope: "personal", creditLimit: "", outstanding: "", dueAmount: "", dueDate: "", currency: "PHP", notes: "" }); setSf(null); };
  const saveEditDebt = () => { const nd = debts.map((d: AnyState) => d.id === eId ? { ...d, ...df, creditLimit: parseN(df.creditLimit), outstanding: parseN(df.outstanding), dueAmount: parseN(df.dueAmount), lastUp: new Date().toISOString() } : d); setDebts(nd); const ns = snap(null, nd); save(null, nd, null, null, null, ns); setEId(null); setSf(null); };
  const quickD = (id: string, f: string, v: string) => { const nd = debts.map((d: AnyState) => d.id === id ? { ...d, [f]: parseN(v), lastUp: new Date().toISOString() } : d); setDebts(nd); save(null, nd); };
  const delDebt = (id: string) => { const nd = debts.filter((d: AnyState) => d.id !== id); setDebts(nd); save(null, nd); };

  const addRecv = () => { if (!rf.name || !rawN(rf.amount)) return; const nr = [...receivables, { id: gid(), ...rf, amount: parseN(rf.amount), received: 0 }]; setReceivables(nr); save(null, null, nr); setRf({ name: "", amount: "", dueDate: "", scope: "personal", currency: "PHP", notes: "", from: "" }); setSf(null); };
  const saveEditRecv = () => { const nr = receivables.map((r: AnyState) => r.id === eId ? { ...r, ...rf, amount: parseN(rf.amount), received: r.received || 0 } : r); setReceivables(nr); save(null, null, nr); setEId(null); setSf(null); };
  const delRecv = (id: string) => { const nr = receivables.filter((r: AnyState) => r.id !== id); setReceivables(nr); save(null, null, nr); };
  const collectRecv = (rv: AnyState) => { const nr = receivables.map((r: AnyState) => r.id === rv.id ? { ...r, received: r.amount } : r); setReceivables(nr); save(null, null, nr); };

  const addCrypto = () => { if (!rawN(crf.amount)) return; const nc = [...cryptos, { id: gid(), ...crf, amount: parseN(crf.amount) }]; setCryptos(nc); const ns = snap(null, null, nc); save(null, null, null, nc, null, ns); setCrf({ coin: "BTC", amount: "", scope: "personal", wallet: "" }); setSf(null); };
  const editCrAmt = (id: string, v: string) => { const nc = cryptos.map((c: AnyState) => c.id === id ? { ...c, amount: parseN(v) } : c); setCryptos(nc); save(null, null, null, nc); };
  const delCrypto = (id: string) => { const nc = cryptos.filter((c: AnyState) => c.id !== id); setCryptos(nc); save(null, null, null, nc); };
  ...
  const addPipeline = () => {
    ...
    setPipelines(np); save(null, null, null, null, null, null, np);
  ...
  const saveEditPipeline = () => {
    ...
    setPipelines(np); save(null, null, null, null, null, null, np); setEId(null); setSf(null);
  };
  ...
  const delPipeline = (id: string) => { const np = pipelines.filter((p: AnyState) => p.id !== id); setPipelines(np); save(null, null, null, null, null, null, np); };

  // Asset CRUD
  const ASSET_CATS: Record<string, string> = { vehicle: "🚗 Vehicle", real_estate: "🏠 Real Estate", equipment: "⚙️ Equipment", electronics: "💻 Electronics", furniture: "🪑 Furniture", other: "📋 Other" };
  const addAsset = () => { if (!axf.name || !rawN(axf.purchasePrice)) return; const na = [...assets, { id: gid(), ...axf, purchasePrice: parseN(axf.purchasePrice), salvageValue: parseN(axf.salvageValue), marketValue: parseN(axf.marketValue), usefulLife: parseFloat(axf.usefulLife) || 5 }]; setAssets(na); save(null, null, null, null, null, null, null, na); setAxf({ name: "", category: "vehicle", purchasePrice: "", purchaseDate: "", usefulLife: "5", salvageValue: "0", marketValue: "", scope: "personal", currency: "PHP", notes: "" }); setSf(null); };
  const saveEditAsset = () => { const na = assets.map((a: AnyState) => a.id === eId ? { ...a, ...axf, purchasePrice: parseN(axf.purchasePrice), salvageValue: parseN(axf.salvageValue), marketValue: parseN(axf.marketValue), usefulLife: parseFloat(axf.usefulLife) || 5 } : a); setAssets(na); save(null, null, null, null, null, null, null, na); setEId(null); setSf(null); };
  const delAsset = (id: string) => { const na = assets.filter((a: AnyState) => a.id !== id); setAssets(na); save(null, null, null, null, null, null, null, na); };

  // Inventory CRUD
  const addInv = () => { if (!ivf.name || !rawN(ivf.qty)) return; const ni = [...inventory, { id: gid(), ...ivf, qty: parseN(ivf.qty), unitCost: parseN(ivf.unitCost), lastUpdated: new Date().toISOString() }]; setInventory(ni); save(null, null, null, null, null, null, null, null, ni); setIvf({ name: "", qty: "", unitCost: "", scope: "business", currency: "PHP", notes: "", sku: "" }); setSf(null); };
  const saveEditInv = () => { const ni = inventory.map((i: AnyState) => i.id === eId ? { ...i, ...ivf, qty: parseN(ivf.qty), unitCost: parseN(ivf.unitCost), lastUpdated: new Date().toISOString() } : i); setInventory(ni); save(null, null, null, null, null, null, null, null, ni); setEId(null); setSf(null); };
  const quickInv = (id: string, f: string, v: string) => { const ni = inventory.map((i: AnyState) => i.id === id ? { ...i, [f]: parseN(v), lastUpdated: new Date().toISOString() } : i); setInventory(ni); save(null, null, null, null, null, null, null, null, ni); };
  const delInv = (id: string) => { const ni = inventory.filter((i: AnyState) => i.id !== id); setInventory(ni); save(null, null, null, null, null, null, null, null, ni); };
  ...
  const savePnlBatch = () => {
    ...
    setPnlBatches(nb); save(null, null, null, null, null, null, null, null, null, nb);
    setCsvPreview(null);
  };
  const delPnlBatch = (id: string) => { const nb = pnlBatches.filter((b: AnyState) => b.id !== id); setPnlBatches(nb); save(null, null, null, null, null, null, null, null, null, nb); };
  ...
  const addTx = () => {
    ...
    setTxns(nt); setAccounts(na); const ns = snap(na, nd); save(na, nd, nr, null, nt, ns);
  ...
  const saveEditTx = () => {
    ...
    setTxns(nt); setAccounts(na); setDebts(nd); setReceivables(nr); save(na, nd, nr, null, nt);
  ...
  const delTx = (tx: AnyState) => {
    ...
    setTxns(nt); setAccounts(na); save(na, nd, nr, null, nt);
  };
...
Important: If you see multiple amounts, use the total/final amount. For bank transfers, the sender pays (expense) and receiver gets (income). If unsure about type, default to "expense".` }
  ...
  const saveScan = () => {
    ...
    const ns = snap(na); save(na, null, null, null, nt, ns);
    setScanResult(null); setScanImg(null);
  };

  // Styles
  const I: React.CSSProperties = { background: "#111318", border: "1px solid #2a2d35", color: "#fff", padding: "10px 12px", borderRadius: 8, fontSize: 14, outline: "none", width: "100%", boxSizing: "border-box" };
  const L: React.CSSProperties = { fontSize: 10, color: "#555", textTransform: "uppercase", letterSpacing: 1, marginBottom: 3, display: "block" };
  const Sec: React.CSSProperties = { fontSize: 12, fontWeight: 700, color: "#555", textTransform: "uppercase", letterSpacing: 2, marginBottom: 10 };
  const Cd = (b: string): React.CSSProperties => ({ background: "#12141A", borderRadius: 10, padding: "12px", borderLeft: `3px solid ${b}` });
  ...
  const CnclS: React.CSSProperties = { background: "none", border: "1px solid #2a2d35", color: "#888", padding: "8px 16px", borderRadius: 8, cursor: "pointer", fontSize: 13 };
  const FmS: React.CSSProperties = { background: "#12141A", border: "1px solid #252830", borderRadius: 10, padding: 16, marginBottom: 14 };
  ...
  if (!loaded) {
    return (
      <div style={{ minHeight: "100vh", background: "#0B0D12", display: "flex", alignItems: "center", justifyContent: "center", color: "#555", fontSize: 14 }}>
        Loading VAULT...
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0B0D12", color: "#ddd", fontFamily: "system-ui, sans-serif", display: "flex" }}>

      {/* ── SIDEBAR ── */}
      <div style={{ width: 220, minWidth: 220, background: "#0a0c10", borderRight: "1px solid #181b22", display: "flex", flexDirection: "column", position: "sticky", top: 0, height: "100vh", overflowY: "auto" }}>
        {/* Logo */}
        <div style={{ padding: "20px 16px 12px" }}>
          <div style={{ fontSize: 20, fontWeight: 800, color: "#fff", letterSpacing: 4 }}>VAULT</div>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 3 }}>
            <span style={{ fontSize: 9, color: "#555", letterSpacing: 2 }}>FINANCE</span>
            <span style={{ fontSize: 8, color: rStat === "live" ? "#95E77E" : "#FFD93D", background: "#1a1d25", padding: "1px 5px", borderRadius: 4 }}>{rStat === "live" ? "● LIVE" : "◌ CACHED"}</span>
          </div>
        </div>
        ...
        <div style={{ flex: 1, padding: "4px 8px" }}>
          {[
            ...
            <button key={k} onClick={() => { setView(k); setSf(null); setEId(null); }}
              style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "10px 12px", marginBottom: 2, background: view === k ? "#12141A" : "transparent", border: "none", borderRadius: 8, color: view === k ? "#fff" : "#555", cursor: "pointer", fontSize: 13, fontWeight: view === k ? 600 : 400, textAlign: "left" as const, borderLeft: view === k ? "3px solid #4ECDC4" : "3px solid transparent" }}>
        ...
        {/* Scope + Settings */}
        <div style={{ padding: "12px 12px 16px", borderTop: "1px solid #181b22" }}>
          <div style={{ fontSize: 9, color: "#555", textTransform: "uppercase" as const, letterSpacing: 1, marginBottom: 6 }}>View</div>
          <div style={{ display: "flex", flexDirection: "column" as const, gap: 3, marginBottom: 12 }}>
            {([["all", "All", "#4ECDC4"], ["personal", "Personal", "#A8B5E2"], ["business", "Business", "#FFD93D"]] as const).map(([k, l, c]) => (
              <button key={k} onClick={() => setScope(k)} style={{ background: scope === k ? "#12141A" : "transparent", border: `1px solid ${scope === k ? c : "transparent"}`, color: scope === k ? "#fff" : "#555", padding: "6px 10px", borderRadius: 6, cursor: "pointer", fontSize: 12, fontWeight: scope === k ? 600 : 400, textAlign: "left" as const }}>{l}</button>
            ))}
          </div>
          <div style={{ fontSize: 9, color: "#555", textTransform: "uppercase" as const, letterSpacing: 1, marginBottom: 6 }}>Currency</div>
          <select value={dCur} onChange={e => setDCur(e.target.value)} style={{ background: "#111318", border: "1px solid #2a2d35", color: "#4ECDC4", padding: "6px 8px", borderRadius: 6, fontSize: 13, outline: "none", fontWeight: 700, width: "100%", marginBottom: 8 }}>{ALL_CUR.map(c => <option key={c}>{c}</option>)}</select>
          <div style={{ display: "flex", gap: 4 }}>
            <button onClick={() => setShowRates(!showRates)} style={{ flex: 1, background: "none", border: "1px solid #2a2d35", color: "#666", padding: "5px", borderRadius: 6, cursor: "pointer", fontSize: 10 }}>💱 Rates</button>
            <button onClick={() => { setAccounts([]); setDebts([]); setReceivables([]); setCryptos([]); setTxns([]); setPipelines([]); setAssets([]); setInventory([]); setPnlBatches([]); setSnaps([]); save([], [], [], [], [], [], [], [], [], []); }} style={{ background: "none", border: "1px solid #2a2d35", color: "#555", padding: "5px 8px", borderRadius: 6, cursor: "pointer", fontSize: 10 }}>⟲</button>
          </div>
          {/* Logout button */}
          <button onClick={handleLogout} style={{ width: "100%", marginTop: 8, background: "none", border: "1px solid #2a2d35", color: "#FF6B6B", padding: "6px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontWeight: 600 }}>Logout</button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", height: "100vh" }}>
        ...
        {view === "dash" && (<div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, marginBottom: 8 }}>
            <div style={Cd("#4ECDC4")}><div style={L}>Fiat</div><div style={{ fontSize: 14, fontWeight: 700, color: "#4ECDC4", marginTop: 3 }}>{fm(tAssets, dCur, true)}</div></div>
            <div style={Cd("#F7931A")}><div style={L}>Crypto</div><div style={{ fontSize: 14, fontWeight: 700, color: "#F7931A", marginTop: 3 }}>{fm(tCrypto, dCur, true)}</div></div>
            <div style={Cd("#FF6B6B")}><div style={L}>Debt</div><div style={{ fontSize: 14, fontWeight: 700, color: "#FF6B6B", marginTop: 3 }}>{fm(tDebt, dCur, true)}</div></div>
          </div>
          {tReceivable > 0 && <div style={{ ...Cd("#26A17B"), marginBottom: 8, display: "flex", justifyContent: "space-between" }}><div><div style={L}>Receivables</div><div style={{ fontSize: 17, fontWeight: 700, color: "#26A17B", marginTop: 3 }}>{fm(tReceivable, dCur, true)}</div></div><div style={{ fontSize: 10, color: "#555", alignSelf: "center" }}>{openR.length} open</div></div>}
          {activeP.length > 0 && <div style={{ ...Cd("#E9C46A"), marginBottom: 8, display: "flex", justifyContent: "space-between" }}><div><div style={L}>Pipeline (Expected Net)</div><div style={{ fontSize: 17, fontWeight: 700, color: "#E9C46A", marginTop: 3 }}>{fm(pipelineNet, dCur, true)}</div></div><div style={{ fontSize: 10, color: "#555", alignSelf: "center" }}>{activeP.length} active · {fm(pipelineGross, dCur, true)} gross</div></div>}
            ...
            {(() => {
              ...
              return (<>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 0 4px", marginTop: 4 }}>
                  ...
                  <div style={{ fontSize: 24, fontWeight: 800, color: total >= 0 ? "#95E77E" : "#FF6B6B" }}>{fm(total, dCur, true)}</div>
                </div>
              ...
              <div style={{ background: "#12141A", borderRadius: 10, padding: "12px 4px 4px" }}>
                <ResponsiveContainer width="100%" height={180}>
                  <AreaChart data={chartData}>
                    ...
                    <YAxis tick={{ fill: "#555", fontSize: 10 }} axisLine={false} tickLine={false} tickFormatter={(v: number) => fm(v, dCur, true)} width={55} />
                    <Tooltip contentStyle={{ background: "#1a1d25", border: "1px solid #2a2d35", borderRadius: 8, fontSize: 12 }} />
          ...
          {sf === "acc" && (
            <div style={FmS}><div style={{ fontWeight: 700, color: "#fff", marginBottom: 12 }}>{eId ? "Edit" : "New Account"}</div>
              <div style={G2}>
                ...
                <div><span style={L}>Scope</span><select style={I} value={af.scope} onChange={e => setAf({ ...af, scope: e.target.value })}><option value="personal">👤 Personal</option><option value="business">📊 Business</option></select></div>
            ...
            sA.map((a: AnyState) => { const c = ACCT_CATS[a.category]; return (
              <div key={a.id} style={{ ...Cd(c?.c || "#555"), marginBottom: 8 }}>
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <div><div style={{ fontSize: 15, fontWeight: 700, color: "#fff" }}>{c?.i} {a.name}</div><div style={{ fontSize: 10, color: "#555", marginTop: 2 }}>{a.scope === "business" ? "📊 BIZ" : "👤 PER"} · {a.currency}</div></div>
        ...
        {view === "crypto" && (<div>
          ...
          {sf === "crypto" && (
            <div style={FmS}><div style={{ fontWeight: 700, color: "#fff", marginBottom: 12 }}>Add Crypto</div>
              <div style={G2}><div><span style={L}>Coin</span><select style={I} value={crf.coin} onChange={e => setCrf({ ...crf, coin: e.target.value })}>{Object.entries(CRYPTO_DEF).map(([k, c]) => <option key={k} value={k}>{c.i} {c.l}</option>)}</select></div><div><span style={L}>Scope</span><select style={I} value={crf.scope} onChange={e => setCrf({ ...crf, scope: e.target.value })}><option value="personal">👤 Personal</option><option value="business">📊 Business</option></select></div><div><span style={L}>Amount</span><NumIn style={I} value={crf.amount} onChange={(v: string) => setCrf({ ...crf, amount: v })} /></div><div><span style={L}>Wallet</span><input style={I} value={crf.wallet} onChange={e => setCrf({ ...crf, wallet: e.target.value })} placeholder="Binance..." /></div></div>
          ...
          {sC.length === 0 ? <div style={{ color: "#333", textAlign: "center", padding: 30, fontSize: 13 }}>No holdings</div> : sC.map((c: AnyState) => { const cr = CRYPTO_DEF[c.coin]; const v = (c.amount * (rates[c.coin] || 0)) / (rates[dCur] || 1); return (
            <div key={c.id} style={{ ...Cd(cr?.c || "#555"), marginBottom: 8 }}>
              <div style={{ display: "flex", justifyContent: "space-between" }}><div><div style={{ fontSize: 16, fontWeight: 700, color: cr?.c }}>{cr?.i} {c.coin}</div><div style={{ fontSize: 11, color: "#555" }}>{c.scope === "business" ? "📊 BIZ" : "👤 PER"}{c.wallet ? ` · ${c.wallet}` : ""}</div></div><div style={{ textAlign: "right" as const }}><div style={{ fontSize: 18, fontWeight: 700 }}>{c.amount} {c.coin}</div><div style={{ fontSize: 13, color: cr?.c, fontWeight: 600 }}>≈ {fm(v, dCur, true)}</div></div></div>
        ...
        {view === "asset" && (<div>
          ...
          {assetSub === "fixed" && (<div>
            ...
            {sf === "asset" && (
              <div style={FmS}><div style={{ fontWeight: 700, color: "#fff", marginBottom: 12 }}>{eId ? "Edit" : "Add Asset"}</div>
                <div style={G2}>
                  ...
                  <div><span style={L}>Market Value (override)</span><NumIn style={I} value={axf.marketValue} onChange={(v: string) => setAxf({ ...axf, marketValue: v })} placeholder="Leave blank for book value" /></div>
                  <div><span style={L}>Scope</span><select style={I} value={axf.scope} onChange={e => setAxf({ ...axf, scope: e.target.value })}><option value="personal">👤 Personal</option><option value="business">📊 Business</option></select></div>
                </div>
            ...
            {sAx.length === 0 ? <div style={{ color: "#333", textAlign: "center", padding: 30, fontSize: 13 }}>No assets</div> : sAx.map((a: AnyState) => {
              ...
              const depAmt = a.purchasePrice - bookVal;
              const years = a.purchaseDate ? ((Date.now() - new Date(a.purchaseDate).getTime()) / (365.25 * 864e5)).toFixed(1) : "?";
              const depPct = a.purchasePrice > 0 ? Math.round((depAmt / a.purchasePrice) * 100) : 0;
              return (
                <div key={a.id} style={{ ...Cd("#D4A373"), marginBottom: 8 }}>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <div>
                      <div style={{ fontSize: 15, fontWeight: 700, color: "#fff" }}>{ASSET_CATS[a.category]?.split(" ")[0] || "📋"} {a.name}</div>
                      <div style={{ fontSize: 10, color: "#555", marginTop: 2 }}>{a.scope === "business" ? "📊 BIZ" : "👤 PER"} · {ASSET_CATS[a.category]?.split(" ").slice(1).join(" ") || a.category} · {years}y old</div>
                    </div>
                    <div style={{ textAlign: "right" as const }}>
                      <div style={{ fontSize: 18, fontWeight: 700, color: "#fff" }}>{fm(displayVal, a.currency)}</div>
                      <div style={{ fontSize: 10, color: a.marketValue > 0 ? "#26A17B" : "#D4A373" }}>{a.marketValue > 0 ? "market value" : "book value"}</div>
                    </div>
                  </div>
                  <div style={{ marginTop: 8, marginBottom: 6 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "#555", marginBottom: 3 }}>
                      ...
                      <span>Cost {fm(a.purchasePrice, a.currency)} → Book {fm(bookVal, a.currency)}</span>
                    </div>
                    <div style={{ height: 5, background: "#1a1d25", borderRadius: 3, overflow: "hidden" }}>
                      <div style={{ height: "100%", width: `${Math.min(depPct, 100)}%`, background: depPct > 80 ? "#FF6B6B" : depPct > 50 ? "#FFD93D" : "#D4A373", borderRadius: 3 }} />
                    </div>
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, marginBottom: 6 }}>
                    ...
                    <div style={{ background: "#0B0D12", borderRadius: 6, padding: "5px 6px" }}><div style={{ fontSize: 9, color: "#555" }}>DEPREC.</div><div style={{ fontSize: 11, fontWeight: 700, color: "#FF6B6B" }}>{fm(depAmt, a.currency)}</div></div>
            ...
            {sf === "inv" && (
              <div style={FmS}><div style={{ fontWeight: 700, color: "#fff", marginBottom: 12 }}>{eId ? "Edit" : "Add Inventory Item"}</div>
                <div style={G2}>
                  ...
                  <div><span style={L}>Scope</span><select style={I} value={ivf.scope} onChange={e => setIvf({ ...ivf, scope: e.target.value })}><option value="personal">👤 Personal</option><option value="business">📊 Business</option></select></div>
                ...
                <div key={i.id} style={{ ...Cd("#CDB4DB"), marginBottom: 8 }}>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <div>
                      ...
                      <div style={{ fontSize: 10, color: "#555", marginTop: 2 }}>{i.sku ? `${i.sku} · ` : ""}{i.scope === "business" ? "📊 BIZ" : "👤 PER"}</div>
                    </div>
                    <div style={{ textAlign: "right" as const }}>
        ...
        {view === "debt" && (<div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 14 }}><div style={Sec}>Cards & Loans</div><button style={BtnS("#FF6B6B", "#fff")} onClick={() => { setSf("debt"); setEId(null); setDf({ name: "", type: "credit_card", scope: scope === "all" ? "personal" : scope, creditLimit: "", outstanding: "", dueAmount: "", dueDate: "", currency: "PHP", notes: "" }); }}>+ Add</button></div>
          {sf === "debt" && (
            <div style={FmS}><div style={{ fontWeight: 700, color: "#fff", marginBottom: 12 }}>{eId ? "Edit" : "Add Debt"}</div>
              <div style={G2}>
                ...
                <div><span style={L}>Scope</span><select style={I} value={df.scope} onChange={e => setDf({ ...df, scope: e.target.value })}><option value="personal">👤 Personal</option><option value="business">📊 Business</option></select></div>
            ...
            return (
              <div key={d.id} style={{ background: "#12141A", borderRadius: 12, padding: "14px", marginBottom: 10, borderLeft: `3px solid ${uc}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                  <div><div style={{ fontSize: 15, fontWeight: 700, color: "#fff" }}>{DEBT_TYPES[d.type]?.i || "💳"} {d.name}</div><div style={{ fontSize: 10, color: "#555", marginTop: 2 }}>{d.scope === "business" ? "📊 BIZ" : "👤 PER"} · {d.currency}</div>{d.dueDate && <div style={{ fontSize: 11, color: uc, fontWeight: 600, marginTop: 3 }}>{days !== null && days < 0 ? `⚠️ OVERDUE ${Math.abs(days)}d` : days === 0 ? "⚡ DUE TODAY" : `📅 ${days}d left`}</div>}</div>
          ...
          {sf === "recv" && (
            <div style={FmS}><div style={{ fontWeight: 700, color: "#fff", marginBottom: 12 }}>{eId ? "Edit" : "Add Receivable"}</div>
              <div style={G2}>
                ...
                <div><span style={L}>Scope</span><select style={I} value={rf.scope} onChange={e => setRf({ ...rf, scope: e.target.value })}><option value="personal">👤 Personal</option><option value="business">📊 Business</option></select></div>
              ...
              <div key={r.id} style={{ background: "#12141A", borderRadius: 10, padding: "14px", marginBottom: 8, borderLeft: `3px solid ${uc}`, opacity: isFullyPaid ? 0.5 : 1 }}>
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: "#555", marginTop: 2 }}>{r.from ? `From: ${r.from} · ` : ""}{r.scope === "business" ? "📊 BIZ" : "👤 PER"}</div>
          ...
          {sf === "pipe" && (
            <div style={FmS}><div style={{ fontWeight: 700, color: "#fff", marginBottom: 12 }}>{eId ? "Edit" : "New Pipeline Entry"}</div>
              <div style={G2}>
                ...
                <div><span style={L}>Scope</span><select style={I} value={pf.scope} onChange={e => setPf({ ...pf, scope: e.target.value })}><option value="personal">👤 Personal</option><option value="business">📊 Business</option></select></div>
              ...
              <div key={p.id} style={{ background: "#12141A", borderRadius: 12, padding: "14px", marginBottom: 10, borderLeft: `3px solid ${isSettled ? "#555" : "#E9C46A"}`, opacity: isSettled ? 0.5 : 1 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: "#555", marginTop: 2 }}>{p.scope === "business" ? "📊 BIZ" : "👤 PER"} · {p.parcels ? `${fmtNum(String(p.parcels))} parcels · ` : ""}{p.date}{p.expectedDate ? ` → ${p.expectedDate}` : ""}</div>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 10, color: "#555", marginTop: 2 }}>
                      {b.platform === "shopee" ? "🟠 Shopee" : b.platform === "tiktok" ? "🎵 TikTok" : b.platform === "shopify" ? "🟢 Shopify" : "📋 Custom"} · {b.date} · {b.scope === "business" ? "📊 BIZ" : "👤 PER"}
                    </div>
                  </div>
                  <div style={{ textAlign: "right" as const }}>
          ...
          {scanResult && !scanResult.error && (
            <div style={{ background: "#12141A", border: "2px solid #4ECDC4", borderRadius: 12, padding: 16, marginBottom: 16 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
                <div style={{ background: "#0B0D12", borderRadius: 8, padding: "10px 12px" }}>
                  <div style={{ fontSize: 10, color: "#555", textTransform: "uppercase" as const }}>Type</div>
                  <div style={{ fontSize: 16, fontWeight: 700, color: scanResult.type === "income" ? "#95E77E" : "#FF6B6B", marginTop: 2 }}>{scanResult.type === "income" ? "↑ Income" : "↓ Expense"}</div>
                </div>
                <div style={{ background: "#0B0D12", borderRadius: 8, padding: "10px 12px" }}>
                  <div style={{ fontSize: 10, color: "#555", textTransform: "uppercase" as const }}>Amount</div>
                  <div style={{ fontSize: 20, fontWeight: 800, color: "#fff", marginTop: 2 }}>{fm(scanResult.amount, scanResult.currency || "PHP")}</div>
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
                <div style={{ background: "#0B0D12", borderRadius: 8, padding: "10px 12px" }}>
                  <div style={{ fontSize: 10, color: "#555", textTransform: "uppercase" as const }}>Source</div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#ccc", marginTop: 2 }}>{scanResult.source || "Unknown"}</div>
                </div>
                <div style={{ background: "#0B0D12", borderRadius: 8, padding: "10px 12px" }}>
                  <div style={{ fontSize: 10, color: "#555", textTransform: "uppercase" as const }}>Date</div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#ccc", marginTop: 2 }}>{scanResult.date}</div>
                </div>
              </div>
              <div style={{ background: "#0B0D12", borderRadius: 8, padding: "10px 12px", marginBottom: 14 }}>
                <div style={{ fontSize: 10, color: "#555", textTransform: "uppercase" as const }}>Description</div>
                <div style={{ fontSize: 13, color: "#ccc", marginTop: 2 }}>{scanResult.description}</div>
                {scanResult.details && <div style={{ fontSize: 11, color: "#555", marginTop: 4 }}>{scanResult.details}</div>}
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
                <div>
                  <span style={{ fontSize: 10, color: "#555", textTransform: "uppercase" as const }}>Personal / Business</span>
                  <select style={{ ...I, marginTop: 4 }} value={scanResult.txScope || "personal"} onChange={e => setScanResult((p: AnyState) => ({ ...p, txScope: e.target.value, category: "" }))}>
                ...
                <div>
                  ...
                  <select style={{ ...I, marginTop: 4 }} value={scanResult.category || ""} onChange={e => setScanResult((p: AnyState) => ({ ...p, category: e.target.value }))}>
                    ...
                    {(TX_CATS[`${scanResult.txScope || "personal"}_${scanResult.type || "expense"}`] || []).map(c => <option key={c.k} value={c.k}>{c.l}</option>)}
                  </select>
                </div>
              </div>
              <div style={{ marginBottom: 14 }}>
                ...
                <select style={{ ...I, marginTop: 4 }} defaultValue={accounts.length > 0 ? accounts[0].id : ""} onChange={e => setScanResult((p: AnyState) => ({ ...p, _accountId: e.target.value }))}>
                  {accounts.map((a: AnyState) => <option key={a.id} value={a.id}>{ACCT_CATS[a.category]?.i} {a.name} ({a.scope === "business" ? "BIZ" : "PER"})</option>)}
              ...
              <div style={FlEnd}>
                ...
                <button onClick={() => {
                  if (scanResult._accountId) {
                    ...
                    const nt = [tx, ...txns];
                    setTxns(nt); setAccounts(na); const ns = snap(na); save(na, null, null, null, nt, ns);
                    setScanResult(null); setScanImg(null);
                  } else { saveScan(); }
              ...
              {scanHistory.slice(0, 10).map((s: AnyState) => (
                <div key={s.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 12px", background: "#12141A", borderBottom: "1px solid #181b22", marginBottom: 1 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 10, color: "#555", marginTop: 1 }}>{s.date} · {s.txScope === "business" ? "BIZ" : "PER"}{s.category ? ` · ${getTxCatLabel(s.category)}` : ""} · {s.confidence}</div>
                  </div>
        ...
        {view === "tx" && (<div>
          ...
          {sf === "tx" && (
            <div style={FmS}><div style={{ fontWeight: 700, color: "#fff", marginBottom: 12 }}>{eId ? "Edit Transaction" : "New Transaction"}</div>
              <div style={G2}>
                <div><span style={L}>{tf.type === "transfer" ? "From Account" : "Account"}</span><select style={I} value={tf.accountId} onChange={e => setTf({ ...tf, accountId: e.target.value })}><option value="">Select...</option>{sA.map((a: AnyState) => <option key={a.id} value={a.id}>{ACCT_CATS[a.category]?.i} {a.name} ({a.scope === "business" ? "BIZ" : "PER"})</option>)}</select></div>
                ...
                {tf.type !== "transfer" && (<>
                  <div><span style={L}>Personal / Business</span><select style={I} value={tf.txScope} onChange={e => setTf({ ...tf, txScope: e.target.value, txCat: "" })}><option value="personal">👤 Personal</option><option value="business">📊 Business</option></select></div>
                  <div><span style={L}>Category</span><select style={I} value={tf.txCat} onChange={e => setTf({ ...tf, txCat: e.target.value })}>
                    <option value="">Select category...</option>
                    {(TX_CATS[`${tf.txScope}_${tf.type}`] || []).map((c: { k: string; l: string }) => <option key={c.k} value={c.k}>{c.l}</option>)}
                  </select></div>
                </>)}
              ...
              <div key={tx.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 12px", background: "#12141A", borderBottom: "1px solid #181b22" }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 11, color: "#444", marginTop: 1 }}>
                    {acc?.name || "?"}{tx.type === "transfer" && toAcc ? ` → ${toAcc.name}` : ""} · {tx.txScope === "business" ? "BIZ" : "PER"} · {tx.date}
